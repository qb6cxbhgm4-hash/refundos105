...
function StorePage() {
  ...
  // Handle OAuth redirect back (shop + session_token in URL params)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const shop = params.get("shop") || localStorage.getItem("ros_pending_shop");
    const token = params.get("session_token") || params.get("token") || params.get("id_token");
    if (shop && token && token !== "standalone") {
      localStorage.removeItem("ros_pending_shop");
      handleConnect(shop, token);
      window.history.replaceState({}, "", window.location.pathname);
    } else if (shop && !token) {
      // Got shop but no token - OAuth may still be in progress
      localStorage.removeItem("ros_pending_shop");
      setError("Authorization incomplete. Please try connecting again.");
    }
  }, []);
  ...
  const startOAuth = () => {
    ...
    // If running inside Shopify admin (embedded app), use App Bridge
    if (window.shopify?.idToken) {
      (async () => {
        setConnecting(true);
        try {
          const token = await window.shopify.idToken();
          handleConnect(shopDomain, token);
        } catch (err) {
          setError("Failed to get session token from Shopify App Bridge.");
          setConnecting(false);
        }
      })();
    } else {
      // Standalone SaaS: redirect to BACKEND auth endpoint
      // Backend initiates Shopify OAuth → Shopify authorizes → redirects back to backend callback
      const API_BASE = "https://animated-space-chainsaw-5gpq7w9g5gqj2vjwq-3000.app.github.dev";
      const authUrl = `${API_BASE}/auth/shopify?shop=${encodeURIComponent(shopDomain)}`;

      setError(null);
      toast("Redirecting to Shopify for authorization...");
      localStorage.setItem("ros_pending_shop", shopDomain);
...
/* ═══ SUPPLIERS ═══ */
const METHODS = [
  { id: "file", label: "File Upload", desc: "Upload CSV or XLSX inventory file from your supplier", icon: sv(<><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="12" y1="18" x2="12" y2="12" /><polyline points="9 15 12 12 15 15" /></>) },
  { id: "sheets", label: "Google Sheets", desc: "Connect a shared Google Sheets URL for live data", icon: sv(<><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="3" y1="9" x2="21" y2="9" /><line x1="3" y1="15" x2="21" y2="15" /><line x1="9" y1="3" x2="9" y2="21" /></>) },
  { id: "link", label: "Data Feed Link", desc: "XML or CSV URL that auto-updates from supplier", icon: sv(<><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></>) },
  { id: "email", label: "Email Order", desc: "Generate purchase orders and send via email", icon: sv(<><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22 6 12 13 2 6" /></>) },
  { id: "api", label: "API / OAuth", desc: "Direct integration for supported suppliers", icon: sv(<><polyline points="16 18 22 12 16 6" /><polyline points="8 6 2 12 8 18" /></>) },
];

const COL_FIELDS = ["Product Code / SKU", "Product Name", "Stock Quantity", "Unit Price", "Lead Time (days)", "MOQ", "Category"];

function SuppliersPage() {
  const toast = useT();
  const [suppliers, setSuppliers] = useState(() => { try { return JSON.parse(localStorage.getItem("ros_suppliers") || "[]"); } catch { return []; } });
  const [adding, setAdding] = useState(false);
  const [editing, setEditing] = useState(null); // supplier id for column mapping

  const save = (list) => { setSuppliers(list); localStorage.setItem("ros_suppliers", JSON.stringify(list)); };
  const addSupplier = (s) => { save([...suppliers, s]); setAdding(false); toast(`${s.name} added`); };
  const removeSupplier = (id) => { save(suppliers.filter(s => s.id !== id)); toast("Supplier removed"); };

  return <div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
      <div><h2 style={{ fontSize: 16, fontWeight: 600, marginBottom: 2 }}>Suppliers</h2><p style={{ fontSize: 12, color: "#6d7175" }}>Manage supplier connections and inventory feeds</p></div>
      {!adding && <Btn primary onClick={() => setAdding(true)}>+ Add supplier</Btn>}
    </div>

    {adding && <AddSupplierFlow onDone={addSupplier} onCancel={() => setAdding(false)} />}

    {editing && <ColumnMapper supplier={suppliers.find(s => s.id === editing)} onSave={(cols) => { save(suppliers.map(s => s.id === editing ? { ...s, mapping: cols } : s)); setEditing(null); toast("Column mapping saved"); }} onCancel={() => setEditing(null)} />}

    {suppliers.length > 0 ? suppliers.map(s => <SupplierCard key={s.id} supplier={s} onRemove={() => removeSupplier(s.id)} onMap={() => setEditing(s.id)} />) :
      !adding && <Card title=""><Empty msg="No suppliers registered. Add your first supplier to start managing inventory feeds." icon={Ic.truck} /></Card>}
  </div>;
}

function AddSupplierFlow({ onDone, onCancel }) {
  const toast = useT();
  const [step, setStep] = useState(1); // 1: name, 2: method, 3: method-specific, 4: done
  const [name, setName] = useState("");
  const [contact, setContact] = useState("");
  const [method, setMethod] = useState(null);
  const [methodData, setMethodData] = useState({});

  const goMethod = () => { if (!name.trim()) { toast("Enter supplier name"); return; } setStep(2); };
  const pickMethod = (m) => { setMethod(m); setStep(3); };
  const finish = () => {
    onDone({
      id: Date.now().toString(),
      name, contact, method: method.id,
  ...
  return <Card title="Add Supplier" right={<Btn slim onClick={onCancel}>Cancel</Btn>}>
    {/* Progress */}
    <div style={{ display: "flex", marginBottom: 24 }}>
      {["Supplier Info", "Connection Method", "Configure", "Done"].map((s, i) => {
        const n = i + 1, a = step >= n;
        return <div key={s} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, position: "relative" }}>
          {i > 0 && <div style={{ position: "absolute", top: 12, right: "50%", width: "100%", height: 2, background: a ? "#008060" : "#e1e3e5", zIndex: 0, transition: "background .3s" }} />}
          <div style={{ width: 24, height: 24, borderRadius: "50%", background: a ? "#008060" : "#e1e3e5", color: a ? "#fff" : "#8c9196", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600, zIndex: 1, transition: "all .3s" }}>{a && step > n ? "v" : n}</div>
          <span style={{ fontSize: 10, color: a ? "#202223" : "#8c9196" }}>{s}</span>
        </div>;
      })}
    </div>
    ...
    {step === 1 && <div style={{ animation: "fadeIn .2s ease" }}>
      <div className="form-g"><div className="form-l">Supplier Name</div><input className="form-i" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Acme Wholesale" autoFocus onKeyDown={e => e.key === "Enter" && goMethod()} /></div>
      <div className="form-g"><div className="form-l">Contact Email (optional)</div><input className="form-i" value={contact} onChange={e => setContact(e.target.value)} placeholder="orders@supplier.com" type="email" /></div>
      <Btn primary onClick={goMethod}>Continue</Btn>
    </div>}

    {/* Step 2: Method selection */}
    {step === 2 && <div style={{ animation: "fadeIn .2s ease" }}>
      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>How do you receive inventory data from this supplier?</div>
      <div style={{ fontSize: 12, color: "#6d7175", marginBottom: 16 }}>Choose the method that matches your current workflow</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {METHODS.map(m => <button key={m.id} onClick={() => pickMethod(m)} className="survey-opt" style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px" }}>
          <div style={{ width: 36, height: 36, background: "#f6f6f7", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#6d7175", flexShrink: 0 }}>{m.icon}</div>
          <div style={{ textAlign: "left" }}><div style={{ fontSize: 14, fontWeight: 600 }}>{m.label}</div><div style={{ fontSize: 12, color: "#6d7175" }}>{m.desc}</div></div>
        </button>)}
      </div>
      <button onClick={() => setStep(1)} style={{ marginTop: 12, background: "none", border: "none", color: "#8c9196", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Back</button>
    </div>}

    {/* Step 3: Method-specific config */}
    {step === 3 && <div style={{ animation: "fadeIn .2s ease" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
        <Badge c="s">{method.label}</Badge>
        <span style={{ fontSize: 13, fontWeight: 600 }}>{name}</span>
      </div>

      {method.id === "file" && <div>
        <div className="form-l" style={{ marginBottom: 8 }}>Upload inventory file</div>
        <div style={{ border: "2px dashed #e1e3e5", borderRadius: 8, padding: 32, textAlign: "center", background: "#fafafa", cursor: "pointer", transition: "all .15s" }}
          onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor = "#008060"; }}
          onDragLeave={e => { e.currentTarget.style.borderColor = "#e1e3e5"; }}
          onDrop={e => { e.preventDefault(); e.currentTarget.style.borderColor = "#e1e3e5"; const f = e.dataTransfer.files[0]; if (f) { setMethodData({ fileName: f.name, fileSize: f.size }); toast(`File: ${f.name}`); } }}
          onClick={() => { const inp = document.createElement("input"); inp.type = "file"; inp.accept = ".csv,.xlsx,.xls"; inp.onchange = (e) => { const f = e.target.files[0]; if (f) { setMethodData({ fileName: f.name, fileSize: f.size }); toast(`File: ${f.name}`); } }; inp.click(); }}>
          <div style={{ color: "#6d7175", marginBottom: 4 }}>{Ic.layers}</div>
          <div style={{ fontSize: 13, fontWeight: 500, color: "#202223" }}>{methodData.fileName || "Drop CSV / XLSX here or click to browse"}</div>
          <div style={{ fontSize: 11, color: "#8c9196", marginTop: 4 }}>Supported: .csv, .xlsx, .xls</div>
        </div>
      </div>}

      {method.id === "sheets" && <div>
        <div className="form-g"><div className="form-l">Google Sheets URL</div><div className="form-h">Make sure the sheet is shared (view access)</div><input className="form-i" value={methodData.url || ""} onChange={e => setMethodData({ url: e.target.value })} placeholder="https://docs.google.com/spreadsheets/d/..." /></div>
        <div className="form-g"><div className="form-l">Sheet Name (optional)</div><input className="form-i" value={methodData.sheet || ""} onChange={e => setMethodData({ ...methodData, sheet: e.target.value })} placeholder="Sheet1" /></div>
      </div>}

      {method.id === "link" && <div>
        <div className="form-g"><div className="form-l">Feed URL</div><div className="form-h">XML or CSV endpoint that returns current inventory</div><input className="form-i" value={methodData.url || ""} onChange={e => setMethodData({ url: e.target.value })} placeholder="https://supplier.com/feed/inventory.csv" /></div>
        <div className="form-g"><div className="form-l">Update Frequency</div>
          <select className="form-i" value={methodData.freq || "daily"} onChange={e => setMethodData({ ...methodData, freq: e.target.value })} style={{ width: "100%" }}>
            <option value="hourly">Every hour</option><option value="daily">Once a day</option><option value="weekly">Once a week</option><option value="manual">Manual only</option>
          </select>
        </div>
      </div>}

      {method.id === "email" && <div>
        <div className="form-g"><div className="form-l">Order Email Address</div><div className="form-h">Where should purchase orders be sent?</div><input className="form-i" value={methodData.email || contact} onChange={e => setMethodData({ email: e.target.value })} placeholder="orders@supplier.com" type="email" /></div>
        <div className="form-g"><div className="form-l">Order Format</div>
          <select className="form-i" value={methodData.format || "pdf"} onChange={e => setMethodData({ ...methodData, format: e.target.value })} style={{ width: "100%" }}>
            <option value="pdf">PDF Purchase Order</option><option value="csv">CSV Attachment</option><option value="text">Plain Text Email</option>
          </select>
        </div>
        <div style={{ background: "#e4f5ee", borderRadius: 8, padding: 12, fontSize: 12, color: "#004c3f" }}>RefundOS will auto-generate purchase orders based on reorder recommendations and send them to this email.</div>
      </div>}

      {method.id === "api" && <div>
        <div className="form-g"><div className="form-l">API Base URL</div><input className="form-i" value={methodData.apiUrl || ""} onChange={e => setMethodData({ ...methodData, apiUrl: e.target.value })} placeholder="https://api.supplier.com/v1" /></div>
        <div className="form-g"><div className="form-l">API Key / Token</div><input className="form-i" value={methodData.apiKey || ""} onChange={e => setMethodData({ ...methodData, apiKey: e.target.value })} placeholder="Bearer token or API key" type="password" /></div>
        <div className="form-g"><div className="form-l">Authentication Method</div>
          <select className="form-i" value={methodData.authType || "bearer"} onChange={e => setMethodData({ ...methodData, authType: e.target.value })} style={{ width: "100%" }}>
            <option value="bearer">Bearer Token</option><option value="basic">Basic Auth</option><option value="apikey">API Key Header</option><option value="oauth">OAuth 2.0</option>
          </select>
        </div>
      </div>}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
        ...
        <Btn primary onClick={() => { setStep(4); setTimeout(finish, 600); }}>Save supplier</Btn>
      </div>
    </div>}

    {/* Step 4: Done */}
    {step === 4 && <div style={{ textAlign: "center", padding: 32, animation: "scaleIn .3s ease" }}>
      <div style={{ width: 36, height: 36, background: "#e4f5ee", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", color: "#008060", fontWeight: 700 }}>OK</div>
      <div style={{ fontSize: 14, fontWeight: 600 }}>Supplier added</div>
      <div style={{ fontSize: 12, color: "#8c9196", marginTop: 4 }}>You can now configure column mapping</div>
    </div>}
  </Card>;
}

function ColumnMapper({ supplier, onSave, onCancel }) {
  const [mapping, setMapping] = useState(supplier?.mapping || {});
  const update = (field, val) => setMapping(p => ({ ...p, [field]: val }));

  return <>
    <div className="modal-overlay" onClick={onCancel} />
    <div className="modal-box" style={{ maxWidth: 520 }}>
      <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Column Mapping</h2>
      <p style={{ fontSize: 12, color: "#6d7175", marginBottom: 20 }}>Map your supplier's columns to RefundOS fields for <strong>{supplier?.name}</strong></p>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {COL_FIELDS.map(field => <div key={field} style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 160, fontSize: 13, fontWeight: 500, flexShrink: 0 }}>{field}</div>
          <input className="form-i" value={mapping[field] || ""} onChange={e => update(field, e.target.value)} placeholder={`Supplier column name for ${field.toLowerCase()}`} style={{ flex: 1 }} />
        </div>)}
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 20, justifyContent: "flex-end" }}>
        <Btn onClick={onCancel}>Cancel</Btn>
        <Btn primary onClick={() => onSave(mapping)}>Save mapping</Btn>
      </div>
    </div>
  </>;
}

function SupplierCard({ supplier, onRemove, onMap }) {
  const [open, setOpen] = useState(false);
  const toast = useT();
  const m = METHODS.find(x => x.id === supplier.method);

  return <div className="card" style={{ marginBottom: 8 }}>
    <div onClick={() => setOpen(!open)} style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
      <div style={{ width: 36, height: 36, background: "#f6f6f7", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#6d7175", flexShrink: 0 }}>{m?.icon || Ic.truck}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 1 }}>{supplier.name}</div>
        <div style={{ fontSize: 12, color: "#8c9196" }}>{m?.label || supplier.method}{supplier.contact ? ` \u00b7 ${supplier.contact}` : ""}</div>
      </div>
      <Badge c={supplier.mapping ? "s" : "w"}>{supplier.mapping ? "Mapped" : "Needs mapping"}</Badge>
    </div>
    {open && <div style={{ padding: "0 16px 14px", borderTop: "1px solid #e1e3e5", paddingTop: 14, animation: "fadeIn .15s ease" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
        {[["Method", m?.label || "—"], ["Added", new Date(supplier.created_at).toLocaleDateString()], ["Status", supplier.mapping ? "Ready" : "Setup needed"]].map(([l, v]) =>
          <div key={l} style={{ background: "#f6f6f7", borderRadius: 6, padding: "8px 10px" }}><div className="kpi-l">{l}</div><div style={{ fontSize: 12, fontWeight: 600 }}>{v}</div></div>
        )}
      </div>

      {/* Method-specific detail */}
      {supplier.data?.url && <div style={{ background: "#f6f6f7", borderRadius: 6, padding: "8px 10px", marginBottom: 12, fontSize: 12, wordBreak: "break-all" }}><span style={{ color: "#8c9196" }}>URL:</span> {supplier.data.url}</div>}
      {supplier.data?.email && <div style={{ background: "#f6f6f7", borderRadius: 6, padding: "8px 10px", marginBottom: 12, fontSize: 12 }}><span style={{ color: "#8c9196" }}>Email:</span> {supplier.data.email}</div>}
      {supplier.data?.fileName && <div style={{ background: "#f6f6f7", borderRadius: 6, padding: "8px 10px", marginBottom: 12, fontSize: 12 }}><span style={{ color: "#8c9196" }}>File:</span> {supplier.data.fileName}</div>}

      <div style={{ display: "flex", gap: 8 }}>
        <Btn slim primary onClick={onMap}>
          {supplier.mapping ? "Edit mapping" : "Set up column mapping"}
        </Btn>
        {supplier.method === "email" && <Btn slim onClick={() => toast("Purchase order draft created")}>Create order</Btn>}
        {(supplier.method === "link" || supplier.method === "sheets") && <Btn slim onClick={() => toast("Sync started")}>Sync now</Btn>}
        <Btn slim danger onClick={onRemove}>Remove</Btn>
      </div>
    </div>}
  </div>;
}
...
/* ═══ NAV CONFIG ═══ */
const TITLES = { home: "Home", products: "Products", detail: "Product Detail", alerts: "Stock Alerts", inventory: "Inventory", analytics: "Analytics", suppliers: "Suppliers", store: "Store", settings: "Settings", account: "Account", support: "Support" };
const NAV = [
  ...
  { id: "suppliers", icon: "truck", label: "Suppliers", sec: "Supply Chain" },
